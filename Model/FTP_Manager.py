...
class FTP_Manager():
    ...
    @staticmethod
    def readLUTFromPLC(ip_address, simulation=False):
        if ip_address == "127.0.0.1" or simulation:
            pixelMappingArray = np.genfromtxt("C:\\ARSimUser\\ControlPLC_Gen3\\PixelMapping\\PrinterPixelMap_DP1_1.vfpmap", delimiter=',', skip_header=1)
        else:
            ftp = FTP_Manager(ip_address, 'admin', 'VFftp')
